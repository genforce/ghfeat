"""Training loop for the hierarchical encoder.

Note that the encoder should be trained after the StyleGAN generator is
well-prepared. For StyleGAN training, please refer to the following repo:
    ...
    with tf.name_scope("Run"), tf.control_dependencies(None):
        ...
        for gpu in range(submit_config.num_gpus):
            with tf.device("/gpu:%d" % gpu):
                in_gpu = in_split[gpu]
                latent_wp = E.get_output_for(in_gpu, is_training=False)
                fake_X_val = Gs.get_output_for(latent_wp, randomize_noise=False)
...
def training_loop(submit_config,
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            G_style_mod = tflib.Network('G_StyleMod', resolution=submit_config.image_size, label_size=0, **G_args)
            ...
            OriD = tflib.Network('D_ori', resolution=submit_config.image_size, label_size=0, **D_args)
            G_style_mod = tflib.Network('G_StyleMod', resolution=submit_config.image_size, label_size=0, **G_args)
            if pretrained_D:
                D = PreD
            else:
    ...
    for gpu in range(submit_config.num_gpus):
        ...
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            G_gpu = G_style_mod if gpu == 0 else G_style_mod.clone(G_style_mod.name + '_shadow')
            feature_model = PerceptualModel(img_size=[E_loss_args.perceptual_img_size, E_loss_args.perceptual_img_size], multi_layers=False)
            ...
            with tf.name_scope('E_loss'), tf.control_dependencies(None):
                E_loss, recon_loss, adv_loss = dnnlib.util.call_func_by_name(E=E_gpu, G=G_gpu, D=D_gpu, feature_model=feature_model, reals=real_gpu, **E_loss_args)
    ...
    for it in range(start, max_iters):
        ...
        _, recon_, adv_, lr = sess.run([E_train_op, E_loss_rec, E_loss_adv, learning_rate], feed_dict)
        if d_scale > 0:
        ...
        if it % 50 == 0:
            print('Iter: %06d/%d, lr: %-.8f recon_loss: %-6.4f adv_loss: %-6.4f run_time:%-12s eta_time:%-12s' % (
                it, max_iters, lr, recon_, adv_,  dnnlib.util.format_time(time.time() - start_time), dnnlib.util.format_time(eta_time)))
            if d_scale > 0:
                print('d_r_loss: %-6.4f d_f_loss: %-6.4f d_reg: %-6.4f ' %(d_r_, d_f_, d_g_))
            sys.stdout.flush()
        ...
        if cur_nimg >= tick_start_nimg + 65000:
            ...
            if cur_tick % image_snapshot_ticks == 0:
                ...
                save_image('%s/iter_%09d.png' % (submit_config.run_dir, cur_nimg), orin_recon)

            if cur_tick % network_snapshot_ticks == 0:
                pkl = os.path.join(submit_config.run_dir, 'network-snapshot-%09d.pkl' % (cur_nimg))
